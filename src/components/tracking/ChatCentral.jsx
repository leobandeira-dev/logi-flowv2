...
export default function ChatCentral({ open, onClose, viagem }) {
  ...
  const loadMensagens = async () => {
    try {
      ...
      // Marcar mensagens como lidas (que não são do usuário atual)
  ...
  const handleEnviarMensagem = async () => {
    ...
    try {
      await base44.entities.Mensagem.create({
        ...
        remetente_tipo: user.role === "admin" ? "central" : "motorista",
        ...
        <DialogHeader className="p-4 border-b">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle>Chat - {viagem.numero_carga || `#${viagem.id.slice(-6)}`}</DialogTitle>
                <p className="text-sm text-gray-500">{viagem.cliente}</p>
              </div>
            </div>
            <Button
              variant="ghost"
                      ...
                      {!minhaMensagem && (
                        <div className="flex items-center gap-2 mb-1">
                          <Badge
                            variant="outline"
                            className={`text-xs ${
                              msg.remetente_tipo === "central" || msg.remetente_tipo === "admin"
                                ? "border-purple-300 text-purple-700 bg-purple-50"
                                : "border-blue-300 text-blue-700 bg-blue-50"
                            }`}
                          >
                            {msg.remetente_tipo === "central" || msg.remetente_tipo === "admin" ? "Central" : "Motorista"}
                          </Badge>
