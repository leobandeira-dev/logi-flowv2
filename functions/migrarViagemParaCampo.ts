...
Deno.serve(async (req) => {
  try {
    ...
    // Buscar todos os campos customizados que sejam "VIAGEM"
    const todosCampos = await base44.asServiceRole.entities.EtapaCampo.list();
    const camposViagem = todosCampos.filter(c => 
      c.nome.toLowerCase().includes('viagem') && !c.nome.toLowerCase().includes('pedido')
    );

    if (camposViagem.length === 0) {
      return Response.json({ 
        success: true, 
        message: 'Nenhum campo VIAGEM encontrado',
        migrados: 0 
      });
    }
    ...
    const valoresViagem = todosValores.filter(v => 
      camposViagem.some(c => c.id === v.campo_id) && v.valor
    );
    ...
    for (const valorCampo of valoresViagem) {
    ...
    for (let i = 0; i < ordemIds.length; i++) {
      try {
        ...
        await base44.asServiceRole.entities.OrdemDeCarregamento.update(ordemId, {
          viagem: updatesPorOrdem[ordemId]
        });
    ...
    return Response.json({
      ...
      migrados: migrados,
      total_valores: valoresViagem.length,
      campos_encontrados: camposViagem.length,
