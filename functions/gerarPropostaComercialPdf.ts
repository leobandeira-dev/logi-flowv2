...
Deno.serve(async (req) => {
  try {
    ...
    // ===== APRESENTAÇÃO COMERCIAL (7 slides) =====
    ...
    doc.setFontSize(24);
    doc.setFont(undefined, 'bold');
    doc.text('Redução de 75% no tempo de cadastro', pageWidth / 2, 165, { align: 'center' });
    doc.setFontSize(16);
    doc.setFont(undefined, 'normal');
    doc.text('De 18 minutos para apenas 4 minutos por ordem', pageWidth / 2, 178, { align: 'center' });
    ...
    doc.setFontSize(36);
    doc.setFont(undefined, 'bold');
    doc.text('Proposta Comercial', pageWidth / 2, 32, { align: 'center' });
    ...
    doc.setFont(undefined, 'normal');
    doc.text(lead.razao_social, 20, 73);
    doc.setFontSize(10);
    doc.setTextColor(75, 85, 99);
    doc.text(`CNPJ: ${lead.cnpj}`, 20, 80);

    // Parse addons
    const leadAddons = lead.addons_selecionados ? JSON.parse(lead.addons_selecionados) : [];

    // Composição do Plano
    doc.setTextColor(17, 24, 39);
    doc.setFontSize(16);
    doc.setFont(undefined, 'bold');
    doc.text('Composição do Plano:', 20, 95);

    // Pacote Base
    yPos = 105;
    doc.setFillColor(240, 249, 255);
    doc.roundedRect(20, yPos, 120, 12, 2, 2, 'F');
    doc.setFontSize(12);
    doc.setFont(undefined, 'bold');
    doc.setTextColor(8, 145, 178);
    doc.text('Pacote Base', 25, yPos + 8);
    doc.setTextColor(17, 24, 39);
    doc.text(`R$ ${(lead.pacote_base_preco || 299).toFixed(2)}`, 135, yPos + 8, { align: 'right' });

    // Add-ons
    yPos += 15;
    if (leadAddons.length > 0) {
      leadAddons.forEach((addon) => {
        doc.setFillColor(240, 249, 255);
        doc.roundedRect(20, yPos, 120, 10, 2, 2, 'F');
        doc.setFontSize(10);
        doc.setFont(undefined, 'normal');
        doc.setTextColor(75, 85, 99);
        doc.text(addon.nome, 25, yPos + 7);
        doc.setTextColor(17, 24, 39);
        doc.text(`R$ ${addon.preco.toFixed(2)}`, 135, yPos + 7, { align: 'right' });
        yPos += 12;
      });
    }
    ...
    yPos += 10;
    doc.setFillColor(6, 182, 212);
    doc.roundedRect(20, yPos, 120, 18, 2, 2, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(16);
    doc.setFont(undefined, 'bold');
    doc.text('TOTAL MENSAL', 25, yPos + 12);
    doc.setFontSize(20);
    doc.text(`R$ ${(lead.valor_total_proposta || 0).toFixed(2)}`, 135, yPos + 12, { align: 'right' });

    // Volumes (lado direito)
    if (lead.volume_coletas || lead.volume_carregamentos || lead.volume_entregas || lead.volume_notas_fiscais) {
      doc.setTextColor(17, 24, 39);
      doc.setFontSize(14);
      doc.setFont(undefined, 'bold');
      doc.text('Volume Mensal:', 160, 95);
      ...
      if (lead.volume_coletas > 0) {
        doc.text(`Coletas: ${lead.volume_coletas}`, 160, yPos);
        yPos += 8;
      }
      if (lead.volume_carregamentos > 0) {
        doc.text(`Carregamentos: ${lead.volume_carregamentos}`, 160, yPos);
        yPos += 8;
      }
      if (lead.volume_entregas > 0) {
        doc.text(`Entregas: ${lead.volume_entregas}`, 160, yPos);
        yPos += 8;
      }
      if (lead.volume_notas_fiscais > 0) {
        doc.text(`Notas Fiscais: ${lead.volume_notas_fiscais}`, 160, yPos);
      }
    }
    ...
    doc.setFontSize(10);
    doc.setTextColor(75, 85, 99);
    doc.text('Proposta válida por 30 dias', pageWidth / 2, pageHeight - 15, { align: 'center' });
    ...
    return new Response(pdfBytes, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Proposta_${lead.razao_social.replace(/[^a-zA-Z0-9]/g, '_')}.pdf"`
      }
    });
  } catch (error) {
